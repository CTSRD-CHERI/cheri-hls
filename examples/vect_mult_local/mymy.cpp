...
bool checkAccess_new(Cap cap, u16 offset, ap_uint<3> nBytes, bool isWrite) {
#pragma HLS INLINE
  return !((cap.base <= (4 * offset)) && ((4 * offset + nBytes) <= cap.top) &&
           (!isWrite || (cap.write)) && (isWrite || (cap.read)));
}
...
#pragma HLS PIPELINE
    ...
    // a
    bool ca = checkAccess_new(caps[0], i, 4, true);
    int a_elem = (ca) ? a[i] : 0;
    // b
    bool cb = checkAccess_new(caps[2], i, 4, true);
    int b_elem = (cb) ? b[i] : 0;
    ...
    // c
    bool cc = checkAccess_new(caps[1], i, 4, true);
    if (cc)
      c[i] = c_elem;

    flag_buf = flag_buf || ca || cb || cc;
  }
