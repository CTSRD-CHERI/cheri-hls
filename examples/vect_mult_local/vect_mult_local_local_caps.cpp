...
void create_cap(int size, LocalCap *caps, int index) {
#pragma HLS INLINE
  LocalCap new_cap;
  new_cap.top = size * 4;
  caps[index] = new_cap;
...
void checkAccessLocal(u32 *flag_buf, LocalCap cap, u64 offset, u64 nBytes) {
#pragma HLS INLINE
  *flag_buf |= !((4 * offset + nBytes) <= cap.top);
}

int cheri_load_local(int *buf, int i, u32 *flag_buf, LocalCap cap) {
#pragma HLS INLINE
  checkAccessLocal(flag_buf, cap, i, 4);
...
#pragma HLS INTERFACE s_axilite port = return
  ...
  Cap caps[2];
  LocalCap localcaps[1];
  u32 buffer[8];
#pragma HLS array_partition variable = buffer type = complete
  //#pragma HLS array_partition variable = caps type = complete
  //#pragma HLS array_partition variable = localcaps type = complete

  load_cap(2, buffer, cap, caps);
  create_cap(N, localcaps, 0); // size, caps array, cap index of b
