...
/* -------------------------------------------------------------------------- */
void aes256_encrypt_ecb(int *key, int *enckey, int *deckey, uint8_t k[32],
                        uint8_t buf[16]) {
  // INIT
  uint8_t rcon = 1;
  uint8_t i;
...
ecb3:
  for (i = 1, rcon = 1; i < 14; ++i) {
    ...
    if (i & 1)
      ...
      aes_expandEncKey(key, &rcon), aes_addRoundKey(buf, key);
  }
  aes_subBytes(buf);
  aes_shiftRows(buf);
...
void hls_top(int size, int key[NUM], int enckey[NUM], int deckey[NUM]) {
...
#pragma HLS INTERFACE s_axilite port = return
  ...
  for (i = 0; i < size; i++) {
    aes256_encrypt_ecb(key + i, enckey + i, deckey + i, key, buf);
  }
}
