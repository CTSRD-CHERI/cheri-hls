...
#pragma HLS array_partition variable = caps type = complete
  ...
  cheri_store(queue, q_in == 0 ? N_NODES - 1 : q_in - 1, starting_node,
              &flag_buf, caps[5]);
  q_in = (q_in + 1) % N_NODES;
...
loop_queue:
  for (dummy = 0; dummy < node; dummy++) { // Typically while(not_empty(queue)){
    // Q_EMPTY()
    if (q_in > q_out ? q_in == q_out + 1
                     : (q_in == 0) && (q_out == N_NODES - 1))
      break;
    // Q_PEEK()
  ...
  loop_neighbors:
    for (e = tmp_begin; e < tmp_end; e++) {
      ...
      if (tmp_level == MAX_LEVEL) { // Unmarked
        ...
        edge_index_t count =
            cheri_load(level_counts, new_level, &flag_buf, caps[10]);
        cheri_store(level_counts, new_level, count + 1, &flag_buf, caps[10]);

        // Q_PUSH(tmp_dst)
        cheri_store(queue, q_in == 0 ? N_NODES - 1 : q_in - 1, tmp_dst,
                    &flag_buf, caps[5]);
        q_in = (q_in + 1) % N_NODES;
      }
    }
