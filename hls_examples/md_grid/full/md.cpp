...
#pragma HLS INTERFACE s_axilite port = return
  ...
  u32 flag_buf = 0;
  // 10 caps: 7 for input arrays + 3 for local arrays (n_points, force,
  // position)
  Cap caps[10];
  u32 buffer[28];
...
#pragma HLS array_partition variable = caps type = complete
  ...
  create_cap(blockSide * blockSide * blockSide * densityFactor, caps,
             8); // force
  create_cap(blockSide * blockSide * blockSide * densityFactor, caps,
             9); // position
    ...
    for (int j = 0; j < size; j++)
      for (int k = 0; k < size; k++)
        for (int h = 0; h < densityFactor; h++) {
          ...
          // Store as struct to force array - need to store each component
          // separately Since we can't directly store structs with cheri_store
          cheri_store((int *)force + idx * 3 + 0, 0, temp_fx, &flag_buf,
                      caps[8]);
          cheri_store((int *)force + idx * 3 + 1, 0, temp_fy, &flag_buf,
                      caps[8]);
          cheri_store((int *)force + idx * 3 + 2, 0, temp_fz, &flag_buf,
                      caps[8]);
          cheri_store((int *)position + idx * 3 + 0, 0, temp_px, &flag_buf,
                      caps[9]);
          cheri_store((int *)position + idx * 3 + 1, 0, temp_py, &flag_buf,
                      caps[9]);
          cheri_store((int *)position + idx * 3 + 2, 0, temp_pz, &flag_buf,
                      caps[9]);
        }
        ...
        loop_grid1_y:
          ...
          loop_grid1_z:
            for (b1.z = MAX(0, b0.z - 1); b1.z < MIN(size, b0.z + 2); b1.z++) {
              ...
              int n_points_idx =
                  b1.x * blockSide * blockSide + b1.y * blockSide + b1.z;
              int q_idx_range =
                  cheri_load(n_points, n_points_idx, &flag_buf, caps[7]);
            loop_p:
              int b0_n_idx =
                  b0.x * blockSide * blockSide + b0.y * blockSide + b0.z;
              for (p_idx = 0;
                   p_idx < cheri_load(n_points, b0_n_idx, &flag_buf, caps[7]);
                   p_idx++) {
                ...
                // Load position
                p.x = cheri_load((int *)position + b0_p_idx * 3 + 0, 0,
                                 &flag_buf, caps[9]);
                p.y = cheri_load((int *)position + b0_p_idx * 3 + 1, 0,
                                 &flag_buf, caps[9]);
                p.z = cheri_load((int *)position + b0_p_idx * 3 + 2, 0,
                                 &flag_buf, caps[9]);

                TYPE sum_x = cheri_load((int *)force + b0_p_idx * 3 + 0, 0,
                                        &flag_buf, caps[8]);
                TYPE sum_y = cheri_load((int *)force + b0_p_idx * 3 + 1, 0,
                                        &flag_buf, caps[8]);
                TYPE sum_z = cheri_load((int *)force + b0_p_idx * 3 + 2, 0,
                                        &flag_buf, caps[8]);
              // For all points in b1
              loop_q:
                for (q_idx = 0; q_idx < q_idx_range; q_idx++) {
                  int q_offset = b1_base_idx + q_idx;
                  q.x = cheri_load((int *)position + q_offset * 3 + 0, 0,
                                   &flag_buf, caps[9]);
                  q.y = cheri_load((int *)position + q_offset * 3 + 1, 0,
                                   &flag_buf, caps[9]);
                  q.z = cheri_load((int *)position + q_offset * 3 + 2, 0,
                                   &flag_buf, caps[9]);
                ...
                cheri_store((int *)force + b0_p_idx * 3 + 0, 0, sum_x,
                            &flag_buf, caps[8]);
                cheri_store((int *)force + b0_p_idx * 3 + 1, 0, sum_y,
                            &flag_buf, caps[8]);
                cheri_store((int *)force + b0_p_idx * 3 + 2, 0, sum_z,
                            &flag_buf, caps[8]);
              } // loop_p
            }
          }
      ...
      for (int k = 0; k < size; k++)
        for (int h = 0; h < densityFactor; h++) {
          int idx = i * blockSide * blockSide * densityFactor +
                    j * blockSide * densityFactor + k * densityFactor + h;
          TYPE force_x_val =
              cheri_load((int *)force + idx * 3 + 0, 0, &flag_buf, caps[8]);
          TYPE force_y_val =
              cheri_load((int *)force + idx * 3 + 1, 0, &flag_buf, caps[8]);
          TYPE force_z_val =
              cheri_load((int *)force + idx * 3 + 2, 0, &flag_buf, caps[8]);
