...
#pragma HLS array_partition variable = caps type = complete
  ...
  create_cap(blockSide * blockSide * blockSide * densityFactor, caps, 8);      // force_ox
  create_cap(blockSide * blockSide * blockSide * densityFactor, caps, 9);      // force_oy
  create_cap(blockSide * blockSide * blockSide * densityFactor, caps, 10);     // force_oz
  create_cap(blockSide * blockSide * blockSide * densityFactor, caps, 11);     // position_ox
  create_cap(blockSide * blockSide * blockSide * densityFactor, caps, 12);     // position_oy
  create_cap(blockSide * blockSide * blockSide * densityFactor, caps, 13);     // position_oz
          ...
          loop_grid1_z:
            for (b1_z = MAX(0, b0_z - 1); b1_z < MIN(size, b0_z + 2); b1_z++) {
              ...
              int b1_base_idx = b1_x * blockSide * blockSide * densityFactor +
                                ...
                                b1_z * densityFactor;
              int b1_n_idx = b1_x * blockSide * blockSide +
                             b1_y * blockSide + b1_z;
              int q_idx_range = cheri_load(n_points, b1_n_idx, &flag_buf, caps[7]);

              int b0_n_idx = b0_x * blockSide * blockSide +
                             b0_y * blockSide + b0_z;
              int p_idx_range = cheri_load(n_points, b0_n_idx, &flag_buf, caps[7]);
            loop_p:
              for (p_idx = 0; p_idx < p_idx_range; p_idx++) {
                ...
                TYPE sum_z = cheri_load(force_oz, b0_p_idx, &flag_buf, caps[10]);
