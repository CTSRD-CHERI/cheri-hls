...
void hls_top(int size, TYPE work_x[512], TYPE work_y[512], u32 *flag, u32 cap[8]) {
...
loop1:
  for (tid = 0; tid < THREADS; tid++) {
    // GLOBAL_LOAD...
    data_x[0] = cheri_load(work_x, 0 * stride + tid, &flag_buf, caps[0]);
    data_x[1] = cheri_load(work_x, 1 * stride + tid, &flag_buf, caps[0]);
    data_x[2] = cheri_load(work_x, 2 * stride + tid, &flag_buf, caps[0]);
    data_x[3] = cheri_load(work_x, 3 * stride + tid, &flag_buf, caps[0]);
    data_x[4] = cheri_load(work_x, 4 * stride + tid, &flag_buf, caps[0]);
    data_x[5] = cheri_load(work_x, 5 * stride + tid, &flag_buf, caps[0]);
    data_x[6] = cheri_load(work_x, 6 * stride + tid, &flag_buf, caps[0]);
    data_x[7] = cheri_load(work_x, 7 * stride + tid, &flag_buf, caps[0]);

    data_y[0] = cheri_load(work_y, 0 * stride + tid, &flag_buf, caps[1]);
    data_y[1] = cheri_load(work_y, 1 * stride + tid, &flag_buf, caps[1]);
    data_y[2] = cheri_load(work_y, 2 * stride + tid, &flag_buf, caps[1]);
    data_y[3] = cheri_load(work_y, 3 * stride + tid, &flag_buf, caps[1]);
    data_y[4] = cheri_load(work_y, 4 * stride + tid, &flag_buf, caps[1]);
    data_y[5] = cheri_load(work_y, 5 * stride + tid, &flag_buf, caps[1]);
    data_y[6] = cheri_load(work_y, 6 * stride + tid, &flag_buf, caps[1]);
    data_y[7] = cheri_load(work_y, 7 * stride + tid, &flag_buf, caps[1]);
...
loop11:
  for (tid = 0; tid < size; tid++) {
    ...
    // Global store
    cheri_store(work_x, 0 * stride + tid, data_x[reversed[0]], &flag_buf, caps[0]);
    cheri_store(work_x, 1 * stride + tid, data_x[reversed[1]], &flag_buf, caps[0]);
    cheri_store(work_x, 2 * stride + tid, data_x[reversed[2]], &flag_buf, caps[0]);
    cheri_store(work_x, 3 * stride + tid, data_x[reversed[3]], &flag_buf, caps[0]);
    cheri_store(work_x, 4 * stride + tid, data_x[reversed[4]], &flag_buf, caps[0]);
    cheri_store(work_x, 5 * stride + tid, data_x[reversed[5]], &flag_buf, caps[0]);
    cheri_store(work_x, 6 * stride + tid, data_x[reversed[6]], &flag_buf, caps[0]);
    cheri_store(work_x, 7 * stride + tid, data_x[reversed[7]], &flag_buf, caps[0]);

    cheri_store(work_y, 0 * stride + tid, data_y[reversed[0]], &flag_buf, caps[1]);
    cheri_store(work_y, 1 * stride + tid, data_y[reversed[1]], &flag_buf, caps[1]);
    cheri_store(work_y, 2 * stride + tid, data_y[reversed[2]], &flag_buf, caps[1]);
    cheri_store(work_y, 3 * stride + tid, data_y[reversed[3]], &flag_buf, caps[1]);
    cheri_store(work_y, 4 * stride + tid, data_y[reversed[4]], &flag_buf, caps[1]);
    cheri_store(work_y, 5 * stride + tid, data_y[reversed[5]], &flag_buf, caps[1]);
    cheri_store(work_y, 6 * stride + tid, data_y[reversed[6]], &flag_buf, caps[1]);
    cheri_store(work_y, 7 * stride + tid, data_y[reversed[7]], &flag_buf, caps[1]);
  }
