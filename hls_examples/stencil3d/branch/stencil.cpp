    ...
    loop_row:
      for (k = 1; k < row_size - 1; k++) {
        ...
        sum1 = cheri_load(orig, INDX(row_size, size, k, j, i + 1), &flag_buf,
                          caps[4]) +
               cheri_load(orig, INDX(row_size, size, k, j, i - 1), &flag_buf,
                          caps[4]) +
               cheri_load(orig, INDX(row_size, size, k, j + 1, i), &flag_buf,
                          caps[4]) +
               cheri_load(orig, INDX(row_size, size, k, j - 1, i), &flag_buf,
                          caps[4]) +
               cheri_load(orig, INDX(row_size, size, k + 1, j, i), &flag_buf,
                          caps[4]) +
               cheri_load(orig, INDX(row_size, size, k - 1, j, i), &flag_buf,
                          caps[4]);
