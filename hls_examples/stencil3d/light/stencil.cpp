...
height_bound_col:
  ...
  height_bound_row:
    for (k = 0; k < row_size; k++) {
      TYPE temp1 = cheri_load(orig, INDX(row_size, size, k, j, 0), &flag_buf, caps[4]);
      cheri_store(sol, INDX(row_size, size, k, j, 0), temp1, &flag_buf, caps[5]);

      TYPE temp2 = cheri_load(orig, INDX(row_size, size, k, j, height_size - 1), &flag_buf, caps[4]);
      cheri_store(sol, INDX(row_size, size, k, j, height_size - 1), temp2, &flag_buf, caps[5]);
    }
  }
col_bound_height:
  ...
  col_bound_row:
    for (k = 0; k < row_size; k++) {
      TYPE temp1 = cheri_load(orig, INDX(row_size, size, k, 0, i), &flag_buf, caps[4]);
      cheri_store(sol, INDX(row_size, size, k, 0, i), temp1, &flag_buf, caps[5]);

      TYPE temp2 = cheri_load(orig, INDX(row_size, size, k, size - 1, i), &flag_buf, caps[4]);
      cheri_store(sol, INDX(row_size, size, k, size - 1, i), temp2, &flag_buf, caps[5]);
    }
  }
row_bound_height:
  for (i = 1; i < height_size - 1; i++) {
  row_bound_col:
    for (j = 1; j < size - 1; j++) {
      TYPE temp1 = cheri_load(orig, INDX(row_size, size, 0, j, i), &flag_buf, caps[4]);
      cheri_store(sol, INDX(row_size, size, 0, j, i), temp1, &flag_buf, caps[5]);

      TYPE temp2 = cheri_load(orig, INDX(row_size, size, row_size - 1, j, i), &flag_buf, caps[4]);
      cheri_store(sol, INDX(row_size, size, row_size - 1, j, i), temp2, &flag_buf, caps[5]);
    }
  }
  ...
  loop_col:
    ...
    loop_row:
      for (k = 1; k < row_size - 1; k++) {
        sum0 = cheri_load(orig, INDX(row_size, size, k, j, i), &flag_buf, caps[4]);
        sum1 = cheri_load(orig, INDX(row_size, size, k, j, i + 1), &flag_buf, caps[4]) +
               cheri_load(orig, INDX(row_size, size, k, j, i - 1), &flag_buf, caps[4]) +
               cheri_load(orig, INDX(row_size, size, k, j + 1, i), &flag_buf, caps[4]) +
               cheri_load(orig, INDX(row_size, size, k, j - 1, i), &flag_buf, caps[4]) +
               cheri_load(orig, INDX(row_size, size, k + 1, j, i), &flag_buf, caps[4]) +
               cheri_load(orig, INDX(row_size, size, k - 1, j, i), &flag_buf, caps[4]);
        TYPE c0 = cheri_load(C, 0, &flag_buf, caps[3]);
        TYPE c1 = cheri_load(C, 1, &flag_buf, caps[3]);
        mul0 = sum0 * c0;
        mul1 = sum1 * c1;
        cheri_store(sol, INDX(row_size, size, k, j, i), mul0 + mul1, &flag_buf, caps[5]);
      }
    }
  }
  for (i = 0; i < row_size * size * height_size; i++) {
