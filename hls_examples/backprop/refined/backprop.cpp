...
#pragma HLS INTERFACE s_axilite port = return
  ...
  u32 flag_buf = 0;
  // 8 arrays (xweights1, xweights2, xweights3, xbiases1, xbiases2, xbiases3,
  // xtraining_data, xtraining_targets) * 4 = 32
  Cap caps[8];
  u32 buffer[32];
...
#pragma HLS array_partition variable = caps type = complete
  ...
  for (int i = 0; i < training_sets; i++)
    for (int j = 0; j < input_dimension; j++)
      training_data[i * input_dimension + j] = cheri_load(
          xtraining_data, i * input_dimension + j, &flag_buf, caps[6]);

  for (int i = 0; i < training_sets; i++)
    for (int j = 0; j < possible_outputs; j++)
      training_targets[i * possible_outputs + j] = cheri_load(
          xtraining_targets, i * possible_outputs + j, &flag_buf, caps[7]);
  ...
  for (int i = 0; i < input_dimension * nodes_per_layer; i++)
    cheri_store(xweights1, i, weights1[i], &flag_buf, caps[0]);
  for (int i = 0; i < nodes_per_layer * nodes_per_layer; i++)
    cheri_store(xweights2, i, weights2[i], &flag_buf, caps[1]);
  for (int i = 0; i < nodes_per_layer * possible_outputs; i++)
    cheri_store(xweights3, i, weights3[i], &flag_buf, caps[2]);

  for (int i = 0; i < nodes_per_layer; i++)
    cheri_store(xbiases1, i, biases1[i], &flag_buf, caps[3]);
  for (int i = 0; i < nodes_per_layer; i++)
    cheri_store(xbiases2, i, biases2[i], &flag_buf, caps[4]);
  for (int i = 0; i < possible_outputs; i++)
    cheri_store(xbiases3, i, biases3[i], &flag_buf, caps[5]);
