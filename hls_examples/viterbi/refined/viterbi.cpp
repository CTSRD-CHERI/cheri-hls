...
#pragma HLS INTERFACE s_axilite port = return
  ...
  u32 flag_buf = 0;
  // 5 arrays (xobs, xinit, xtransition, xemission, xpath) * 4 = 20
  Cap caps[5];
  u32 buffer[20];
...
#pragma HLS array_partition variable = caps type = complete
  ...
  for (int i = 0; i < N_OBS; i++)
    obs[i] = cheri_load(xobs, i, &flag_buf, caps[0]);
  for (int i = 0; i < N_STATES; i++)
    init[i] = cheri_load(xinit, i, &flag_buf, caps[1]);
  for (int i = 0; i < N_STATES * N_STATES; i++)
    transition[i] = cheri_load(xtransition, i, &flag_buf, caps[2]);
  for (int i = 0; i < N_STATES * n_tokens; i++)
    emission[i] = cheri_load(xemission, i, &flag_buf, caps[3]);
...
L_init:
  for (s = 0; s < N_STATES; s++) {
    llike[0 * N_STATES + s] = init[s] + emission[s * n_tokens + obs[0]];
  }
...
L_backtrack:
  ...
  for (int i = 0; i < N_OBS; i++)
    cheri_store(xpath, i, path[i], &flag_buf, caps[4]);
