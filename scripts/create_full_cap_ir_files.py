...
execute(["bash", "/workspace/scripts/run-vitis-hls.sh", "mem2reg_full.tcl"])
execute(["bash", "/workspace/scripts/run-vitis-hls.sh", "pre_full.tcl"])
...
with open("/workspace/scripts/cheri_resources/full_cap_header.ll", "w") as f:
    for line in header:
        f.write(line.rstrip() + "\n")

with open("/workspace/scripts/cheri_resources/full_cheri_attributes.ll", "w") as f:
